...
# Load and filter
signal = DetectorData(r'Z:\Studenten\Bakhodirov\coincidence_analyzer-1\coincidence_analysis\Detector_data\AlSurf280um_listmode.txt')
background = DetectorData(r'Z:\Studenten\Bakhodirov\coincidence_analyzer-1\coincidence_analysis\Detector_data\background_listmode.txt')

df_signal = signal.get_dataframe()
analyzer = CoincidenceAnalyzer(df_signal)

df_bg = background.get_dataframe()
analyzer_bg = CoincidenceAnalyzer(df_bg)

# Δt histogram and window selection
delta_ts = analyzer.coincidence_search(
    dt_window_ns=(-200, 200),
    return_delta_t=True
)
